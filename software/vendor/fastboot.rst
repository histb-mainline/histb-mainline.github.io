...
**NOTE!** It's not Android Fastboot as you might image, though it can actually boot Android bootimgs. Clearly it's U-Boot forked around 2014, and removed any copyright notice.

Vendor fastboot support booting from uImage or Android bootimg (uImage + initrd).

Boot sequence:

#. If ``USB_BOOT`` is pulled down, it will try to read two files on u-disk:
...
#. Read Android bootimg or uImage from eMMC or NAND storage. If Secure Boot is enabled, only Android bootimg will be considered.
...
For most commands, refer to U-Boot help pages.

Make uImage or Android bootimg
------------------------------
